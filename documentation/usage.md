...
This is the primary installation path for **x86-64 CHR, IPv4,
RouterOS 7.23+**. The router performs installation and management itself.
Tested locally on **7.23.2 and 7.23.5**; see [acceptance results](archive/routeros-tests.md).
...
The container image is **93,635,286 bytes compressed** and about **264.7 MB unpacked**.
...
[mikrowarp-linux-amd64.tar.gz](https://github.com/parhamfa/mikrowarp/releases/download/v1.0.0/mikrowarp-linux-amd64.tar.gz).
Each released installer selects its matching image. Downloads use
...
logs and diagnostic retention remain bounded by the container.
...
application. MikroWARP includes no destination-specific relay or exit-region
workaround; see the [project scope](#scope).

## Scope

MikroWARP provides Cloudflare's official WARP client, IPv4 forwarding, health
monitoring and recovery on supported MikroTik CHRs. Packaging, forwarding and
monitoring defects remain project issues.

Destination reachability also depends on Cloudflare, the uplink and the remote
service. Sampled health does not guarantee every destination or a fixed exit
region. The NAT64 and Psiphon experiments are retained in the
[research archive](archive/README.md); neither is included in the container.

Long-term uptime, production throughput and varied upstream paths remain outside
the recorded lab validation. This release supports x86-64 CHR with IPv4; it does
not claim physical-router, ARM or IPv6 forwarding support.
