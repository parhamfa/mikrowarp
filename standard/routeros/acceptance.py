...
from build import DEFAULT, render
...
class Acceptance:
    ...
    def repeat(self):
        ...
        state = self.journal()
        self.native(script='sibling.rsc' if state['current']['image_id'] != DEFAULT['image_id'] else 'mikrowarp.rsc')
        self.native('status')
    ...
    def terminal_disconnect(self):
        ...
        target = sibling if self.image() == DEFAULT['image_id'] else DEFAULT
    ...
    def interrupted_update(self, phase):
        ...
        registration = self.reg()
        sibling = json.loads((OUT / 'sibling.json').read_text())
        target = sibling if before['current']['image_id'] == DEFAULT['image_id'] else DEFAULT
