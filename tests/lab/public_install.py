...
from .transport import quote

parser = argparse.ArgumentParser(description=__doc__)
parser.add_argument('--installer-url', default='https://github.com/parhamfa/mikrowarp/releases/latest/download/mikrowarp.rsc')
parser.add_argument('--expected-installer', type=Path, default=ROOT / 'mikrowarp.rsc')
parser.add_argument('--metadata', type=Path, help='Candidate metadata.json; omitted for the current release')
args = parser.parse_args()
release = json.loads(args.metadata.read_text()) if args.metadata else DEFAULT
expected_installer = args.expected_installer.read_text()
assert expected_installer == render(release), 'Expected installer differs from the chosen source and metadata'
assert args.installer_url.startswith('https://'), 'Public installation requires HTTPS'

t = Acceptance(release)
...
fetch = ('/tool fetch url=' + quote(args.installer_url) +
         ' dst-path=mikrowarp.rsc check-certificate=yes http-max-redirect-count=5')
started = time.monotonic()
print('Fresh local CHR: fetching the public installer', flush=True)
t.r.run(fetch, timeout=120)
script = t.read('mikrowarp.rsc') + '\n'
assert script == expected_installer
...
t = Acceptance(release)
assert t.image() == release['image_id']
assert t.ready()
t.stable_footprint()
after = export()
